...
export const PopoverCard: React.FC<PopoverCardProps> = ({
  ...
  const resetCardStyle = () => {
    if (cardRef.current) {
      ...
      (
        cardRef.current.style as CSSStyleDeclaration & { webkitBackdropFilter: string }
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: 100 + (popup.depth ?? 0),
        transition: activeMove
          ? 'none'
          : 'transform 0.75s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s ease',
    ...
    >
      ...
      <div className="w-full h-full flex flex-col popup-inner">
        ...
        <h5 className="font-human text-base font-bold text-black mb-1.5 px-4 pt-4 shrink-0">
          {popup.title}
        </h5>
            ...
            {popup.id === 'heine-borel' ? (
              <>
                ...
                <span
                  ...
                  onMouseEnter={(e) => onLinkHover(e, 'compactness', popup.depth ?? 0)}
              ...
              <>
                Asserts that the product of any collection of compact spaces is compact. It is
                equivalent to the{' '}
                <span
                  className="wiki-link font-bold"
                  onMouseEnter={(e) => onLinkHover(e, 'axiom-of-choice', popup.depth ?? 0)}
